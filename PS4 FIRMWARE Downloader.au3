...
#AutoIt3Wrapper_Icon=287672_playstation_icon.ico
...
If StringInStr($firmware_dl,"rec") Then
	$update_zip = $update_dir&"\reinstallation\PS4UPDATE.PUP"
	$text = "Re-Installation"
EndIf
If StringInStr($firmware_dl,"sys") Then
...
If $online_update <> $cache_update Then
	...
	Select
		Case $iMsgBoxAnswer = 1 ;OK
			ConsoleWrite("Downloading "&$text&" File........"&@CRLF)
			...
			ProgressOn($TITLE, "Downloading "&$text&" File", "0%", -1, -1, BitOR($DLG_NOTONTOP, $DLG_MOVEABLE))
			Do
	...
	EndSelect
Else
SplashTextOn($TITLE, "PS4 "&$text&" File up to date", 300, 70, -1, -1, $DLG_TEXTVCENTER, "", 24)
Sleep(3000)
SplashOff()
EndIf
EndFunc
